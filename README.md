...
On Safari the issue was that the hero-video on the "Home page" wasn't visible. After trying to add controls, playsinline without success, 
...
- **Call to action button** "Read more about us": hovering over it turns into Porcelain color, the same color of the navbar and the footer. If pressed it links and brings to the "About us" page.
...
- On "Gallery" page at the beginning I placed 6 images every row divided into categories on large screens and col-6 on small screens but they seemed to me too many pictures. So I decided to use Fancybox preview which allowed me to display only three images per row on large screens and hide the rest of the pictures. Only if we click an image Fancybox viewer let us see all the collection.
